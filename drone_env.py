...
# ---------------------------------------------------------------------------
# Note on experiment configurations
# ---------------------------------------------------------------------------
#
# This file intentionally does NOT define named tasks such as free_random,
# one_obstacle_fixed, three_obstacles_random, or wind variants.
#
# All experiment-specific task definitions, reward settings, obstacle layouts,
# and curriculum aliases should live in env_configs.py. This keeps the base
# environment framework-agnostic and prevents SAC, SB3 PPO, and custom PPO
# from accidentally using different task definitions.
#
# If DroneNavEnv is created without a config, it falls back to EnvConfig(),
# which is only a minimal empty-world default for debugging/smoke tests.
...
class DroneNavEnv(gym.Env):
    ...
    def __init__(
        ...
        if config is None:
            # Minimal empty-world default. Final experiments should pass an
            # explicit config from env_configs.make_config(...).
            config = EnvConfig()
...
if __name__ == "__main__":
    env = DroneNavEnv(config=EnvConfig(wind_std=0.0, n_rays=16, ray_max_dist=10.0))
